...
def reset_preferences() -> str:
    """Reset and clear all saved travel preferences for the user"""
    if not memory_client or not MEMORY_ID:
        return "Memory not configured - set MEMORY_ID environment variable"
    ...
    try:
        memory_client.create_event(
            ...
            messages=[
                ("Please forget all my previous travel preferences including hotel type, food restrictions, budget, and interests. I want to start fresh.", "USER"),
                ("All your travel preferences have been cleared. You are starting fresh with no stored preferences.", "ASSISTANT")
            ]
        )
        return "All preferences cleared. Starting fresh."
    except Exception as e:
...
- Resetting all preferences when user asks to start fresh (reset_preferences)
